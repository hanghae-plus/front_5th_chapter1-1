...
const ErrorPage = () => {
  ...
  const onRendered = () => {
    ...
    const handleClickHomeLink = (event) => {
      event.preventDefault();

      router.navigate(ROUTE_PATHS.HOME);
    };

    link.addEventListener("click", handleClickHomeLink);

    registerCleanup("errorPage", () => {
      link.removeEventListener("click", handleClickHomeLink);
    });
  };
